...
summary: 面向平台管理员的后台查询与标签、话题管理接口，含仪表盘统计与操作日志只读查询。
---

## 1. 模块概述

后台管理模块（admin）挂载在 `/api/v1/admin` 路由组下，全部接口要求 JWT 登录且角色为 `admin`。模块提供用户、帖子、评论、圈子、操作日志的分页查询，仪表盘四类实体总量统计，以及标签与话题的创建和更新能力。内容审核写操作（帖子/评论下架、删除）、用户封禁与角色变更未在本模块实现，用户与内容类接口均为只读。
...
- 用户列表复用 `UserRepository.List`：固定过滤 `status <> 'banned'`（被封禁用户在后台列表不可见），`keyword` 对 `nickname`、`account` 做 LIKE 匹配，按 `id ASC` 排序；返回的 `dto.User` 不携带关注状态。
...
- 无用户封禁/解禁、角色变更、用户详情接口；用户封禁状态机不在本模块（后台无法看到也无法产生 `banned` 用户）。
- 无帖子/评论的下架、恢复、删除接口；内容审核流转（pending/approved/rejected 之类）不存在，帖子仅能按已有状态只读过滤。
- 无圈子管理写接口（创建、解散、推荐位设置均无）。
- `OperationLog` 只有读取路径：`AdminRepository` 仅实现 `ListLogs`，后端代码中无任何写入 `operation_logs` 的调用，正常运链路下该表为空，日志列表接口返回空数据。
...
跨模块操作：`AdminService` 除自有 `AdminRepository`（帖子列表、日志列表、统计）外，直接注入并调用 `UserRepository`、`TagRepository`、`TopicRepository`、`CircleRepository`、`CommentRepository` 完成查询与标签、话题写入；不经过各业务模块的 Service 层。`operation_logs` 已注册在 `model/all.go` 的自动迁移列表中。
