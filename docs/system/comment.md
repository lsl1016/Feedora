...
summary: 评论模块提供帖子评论的发表、楼中楼回复、软删除、点赞与树形列表查询，评论计数在事务内同步维护，并通过 comment.events 事件驱动通知、积分与热度榜单。
---
...
| `/comments/:commentId` | DELETE | 删除评论（软删除） | `CommentAPI.Delete` |
...
- `ListByPost` 一次性查出帖子全部 `status = normal` 的评论（`created_at` 升序，无分页），先构建根评论数组并建立 `评论 ID → 数组下标` 索引，再将 `parent_id != 0` 的评论按 `root_id`（为 `0` 时回退取 `parent_id`）挂到对应根评论的 `replies` 数组，回复内部同样按创建时间升序。
- 呈现结构固定为两层：所有子回复平铺在根评论的 `replies` 下，不构建第三层及以上嵌套。
- 若某条子回复的根评论已被删除（不在查询结果中），该子回复在组装时被丢弃，不返回给前端。
...
- 回复前校验父评论存在（`FindByID` 受 GORM 软删过滤，已删评论返回 `ErrCommentNotFound`）。
...
软删语义：删除时同时写 `status = deleted` 与 `deleted_at` 两套标记；查询侧为双重过滤——GORM 自动排除 `deleted_at` 非空记录，业务查询再过滤 `status = normal`。点赞关系表 `comment_likes` 不随评论删除而清理。
